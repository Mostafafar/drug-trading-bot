...
async def handle_admin_edit_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if query.data == "admin_edit_name":
            ...
            context.user_data['_conversation_state'] = States.ADMIN_EDIT_DRUG_NAME  # 🔥 اضافه شد
            logger.info(f"State set to: {States.ADMIN_EDIT_DRUG_NAME}")
            ...
            context.user_data['original_price'] = raw_price
            context.user_data['_conversation_state'] = States.ADMIN_EDIT_DRUG_PRICE  # 🔥 اضافه شد
            logger.info(f"State set to: {States.ADMIN_EDIT_DRUG_PRICE}")
            ...
            context.user_data['_conversation_state'] = States.ADMIN_EDIT_DRUG  # 🔥 اضافه شد
...
async def handle_admin_edit_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if query.data == "admin_back_to_list":
            ...
            # ایجاد یک update مصنوعی برای بازگشت به لیست
            fake_update = Update(
                update_id=update.update_id,
                message=Message(
                    message_id=update.update_id,
            ...
            return await admin_edit_drugs(fake_update, context)
        ...
        if query.data == "admin_cancel_delete":
            ...
            try:
                ...
                try:
                    conn = get_db_connection()
                    with conn.cursor() as cursor:
                        cursor.execute('DELETE FROM drug_items WHERE name = %s', (drug['name'],))
                        conn.commit()
                except Exception as e:
                    logger.error(f"Error deleting drug from DB: {e}")
                    if conn:
                        conn.rollback()
                finally:
                    if conn:
    ...
    try:
        ...
        try:
            ...
            if not cleaned_price:
                logger.warning("Invalid price: empty after cleaning")
                await update.message.reply_text("❌ لطفا یک قیمت معتبر وارد کنید:")
                return States.ADMIN_EDIT_DRUG_PRICE
                
            price_int = int(cleaned_price)
            if price_int <= 0:
                logger.warning("Invalid price: zero or negative")
            ...
            logger.error(f"Price validation error: {ve}")
            await update.message.reply_text("❌ لطفا یک عدد معتبر وارد کنید:")
            return States.ADMIN_EDIT_DRUG_PRICE
        ...
        for i, (name, price) in enumerate(drug_list):
            if name == editing_drug['name']:
                ...
                updated = True
                logger.info(f"Updated drug_list: {name} -> {formatted_price}")
                break
        if not updated:
            logger.warning(f"Drug {editing_drug['name']} not found in drug_list")
...
async def show_admin_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        keyboard = [
            ...
            ['💊 ویرایش داروها', '📤 آپلود اکسل'],
            ['✅ تأیید کاربران', '🔙 منوی اصلی']
        ]
        reply_markup = ReplyKeyboardMarkup(keyboard, resize_keyboard=True)
        
        if update.message:
            await update.message.reply_text(
                "به پنل ادمین خوش آمدید. لطفاً یک گزینه را انتخاب کنید:",
                reply_markup=reply_markup
            )
        else:
            await context.bot.send_message(
