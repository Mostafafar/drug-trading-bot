...
async def handle_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            conn = get_db_connection()
            with conn.cursor(cursor_factory=extras.DictCursor) as cursor:
                # Search in both predefined drugs and user-added drugs
                cursor.execute('''
                SELECT 
                    di.id, 
                    di.user_id,
                ...
                if results:
                    ...
                    # If no results found, suggest adding the drug
                    keyboard = [
                        [InlineKeyboardButton("➕ ثبت این دارو", callback_data=f"add_drug_{search_term}")],
                        [InlineKeyboardButton("🔙 بازگشت", callback_data="back")]
                    ]
                    
                    await update.message.reply_text(
                        "هیچ دارویی با این نام یافت نشد.\n\n"
                        "اگر این دارو را در اختیار دارید، می‌توانید آن را به لیست اضافه کنید.",
                        reply_markup=InlineKeyboardMarkup(keyboard)
                    )
                    return ConversationHandler.END
