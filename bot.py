...
async def handle_drug_selection_from_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """پردازش انتخاب دارو از کیبورد با فرمت جدید"""
    try:
        ...
        elif selection == "📌 داروهای هدف":
            context.user_data['current_list_type'] = 'target'
            context.user_data['page_target'] = 0
            return await show_two_column_selection(update, context)
        elif selection == "💊 داروهای من":
        ...
        # پردازش انتخاب دارو - فرمت جدید: "نام دارو (تعداد)"
        try:
            # پیدا کردن داروی انتخاب شده بر اساس نام
            selected_drug = None
            selected_index = -1
            
            for i, drug in enumerate(drugs):
                # ساخت متن دکمه مطابق با فرمت جدید
                drug_name = drug['name']
                if len(drug_name) > 20:
                    drug_name = drug_name[:17] + "..."
                expected_text = f"{drug_name} ({drug['quantity']})"
            ...
            if selected_drug:
                ...
                # اگر با نام مطابقت نداشت، شاید کاربر عدد وارد کرده
                try:
                    # تبدیل اعداد فارسی به انگلیسی
                    persian_to_english = str.maketrans('۰۱۲۳۴۵۶۷۸۹', '0123456789')
                    ...
                    # استخراج عدد از متن
                    index_str = ''.join(filter(str.isdigit, clean_selection))
        ...
        await update.message.reply_text(
            "❌ لطفا یک گزینه معتبر از لیست انتخاب کنید.\n\n"
            "مثال:\n"
            "- روی دکمه 'نام دارو (تعداد)' کلیک کنید\n"
